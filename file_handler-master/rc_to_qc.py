...
def t_general(temp_gate, venus, iter_counter):
    ...
    with open('converted.real', 'a') as lake:
        ...
        for emo in range(len(canatrus_centuri)):
            gi_joe = str(canatrus_centuri[emo])
            gi_joe = re.split(',', gi_joe)
        ...
        for cario in range(len(canatrus_centuri)):
            amigo = str(canatrus_centuri[cario])
            amigo = re.split(',', amigo)
...
with open('main1.tfc', 'r+') as file_alpha:
...
with open('converted.real', 'a') as cosmos:
    if "'" in list_1:
        mon = list_1.index("'")
        del list_1[mon]
        # print('list_1 :', list_1)
